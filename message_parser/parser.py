...
class CommandModule:
    """
    Базовый класс для классов, которые хранят комманды бота
    """
...
class CommandDescription:
    ...
    def __init__(self, command_name: str, action, help_info: str, visibility: bool = True, access_level: int = 10,
                 args_info: str = None):
        if ' ' in command_name:
...
class MessageParser(object):
    ...
    def __init__(self, command_symbol: str = '/', admin_pincode: int = 1):
        ...
        self.add_command('help', action=self.list_commands, help_info='Lists all commands', args_info="admin pin code")
    ...
    def list_commands(self, event: Event, pin: int = 0, *args):
        ...
        for command in self.commands.values():
            if command.visibility or (pin == self.admin_pincode):
                info = f'{self.command_symbol}{command.name} - {command.help_info}'
    ...
    def add_module_commands(self, commands: dict):
        ...
        if len(intersection) > 0:
            raise ValueError(f'duplicate commands {str(intersection)}')
        for command in commands:
            if type(commands[command]) is not CommandDescription:
                raise ValueError('command must be CommandDescription class')
        self.commands.update(commands)

    def load_module_commands(self, module: object):
        gg = vars(module)
        if hasattr(module, "commands"):
            self.add_module_commands(module.commands)
        else:
            raise ValueError(f'module {module.__str__()} have not commands variable')
    ...
    def add_command(self, command_name: str, action, visibility: bool = True, help_info: str = '',
                    args_info: str = None):
        ...
        self.commands[command_name] = CommandDescription(command_name, action, help_info, visibility, args_info)
